...
test('Renders the <Nav /> and the <Messages /> when a username is set', () => {
	const { getByText, queryByTestId, getByPlaceholderText, getByRole } = render(<App />);
...
test('Renders the <Login /> again if the user clicks the logout button after being authentified', () => {
	const { queryByTestId, getByPlaceholderText, getByRole, getAllByRole } = render(<App />);
	...
	const logoutButton = getAllByRole('button')[0];
